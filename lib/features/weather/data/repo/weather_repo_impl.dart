...
class WeatherRepoImpl implements WeatherRepoContract {
  ...
  @override
  Future<BaseResponse<WeatherResponseEntity>> getWeatherByCity(
    String cityName,
  ) async {
    final response = await _dataSource.getCityWeather(cityName: cityName);
    return response.when(
      success: (dto) {
        final WeatherResponseEntity entity = dto.toEntity();
        return BaseResponse<WeatherResponseEntity>.success(entity);
  ...
  Future<BaseResponse<ForecastResponseEntity>> getForecastByCity(
    ...
    final response = await _dataSource.getForcasetWeather(cityName: cityName);
    return response.when(
      success: (dto) {
        final ForecastResponseEntity entity = dto.toEntity();
        return BaseResponse<ForecastResponseEntity>.success(entity);
